...
function botScoreAir(G, p, did) {
  const f = distInfo(did).f, a = C(p.prep.actor);
  const q = previewQuality(G, p), b = a.buzz;
  return evalExpr(f.fame, q, b) * 3 + evalExpr(f.money, q, b) + evalExpr(f.aware, q, b) * 1.5;
}
...
function botStep(G) {
  ...
  if (p.crews.length < 3 && p.money >= 5) { const i = b.crew.findIndex((o, k) => can('crew', k)); const gs = botGenres(p);
    if (i >= 0) { const id = G.market.crew.filter(id => id && C(id).id !== 'C09' && C(id).price <= p.money - 1).sort((x, y) => { const f = c => (/모든 작품/.test(C(c).effect) || gs.some(g => C(c).effect.includes(g)) ? 2 : 0) - C(c).price * 0.1; return f(y) - f(x); })[0]; if (id) return place(G, 'crew', i, { card: id, replace: 0 }); } }
  // 5. awareness
